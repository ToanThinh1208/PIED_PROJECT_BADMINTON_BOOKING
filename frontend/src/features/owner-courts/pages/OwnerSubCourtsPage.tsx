    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {selectedCourtId && !isScheduleMode && (
            view === "list" ? (
              <Button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {isLoadingCourts ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl border border-gray-100 p-6 h-48 animate-pulse flex flex-col gap-4">
            ...
            <div className="col-span-full py-20 flex flex-col items-center justify-center bg-white rounded-3xl border border-gray-100 border-dashed">
              ...
              <p className="text-gray-400 font-medium">Bạn chưa có cơ sở nào được đăng ký</p>
              <Button 
                variant="link" 
          ...
          <div className="space-y-4">
            {view === "list" ? (
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 max-w-4xl mx-auto space-y-10">
                <div className="space-y-2">
                  <h2 className="text-xl font-bold text-gray-900">Thông tin sân con mới</h2>
                  <p className="text-sm text-gray-400">Vị trí: <span className="text-emerald-600 font-semibold">{selectedCourt?.name}</span></p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <Label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Tên sân con</Label>
                    <Input
                  ...
                  <div className="space-y-3">
                    <Label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Giá niêm yết (VNĐ/Giờ)</Label>
                    <Input
                      type="number"
